...
CHANNEL_USERNAME = '@W_S_B52' # معرف قناتك
...
def send_welcome(message):
    bot.reply_to(message, f"أهلاً وسام! أرسل الـ PDF وسأترجمه لك مع الصور.\nيجب أن تكون مشتركاً في: {CHANNEL_USERNAME}")
...
def handle_docs(message):
    ...
    if not is_subscribed(user_id):
        ...
        markup.add(btn)
        bot.reply_to(message, f"🚫 عذراً، يجب عليك الاشتراك في القناة لاستخدام البوت:\n{CHANNEL_USERNAME}", reply_markup=markup)
        return
    ...
    msg = bot.reply_to(message, "⏳ جاري استخراج الصور وترجمة النصوص...")
        ...
        for page in doc:
            ...
            for img_index, img in enumerate(page.get_images(full=True)):
                try:
                    ...
                    # تجاهل الصور الصغيرة جداً (مثل الأيقونات) لسرعة المعالجة
                    ...
                    # وضع الصورة بمنتصف الصفحة تقريباً
