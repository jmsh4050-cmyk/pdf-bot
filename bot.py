...
def send_welcome(message):
    bot.reply_to(message, f"أهلاً وسام! أرسل ملف PDF واختار شكل الترجمة.\nقناتنا: {CHANNEL_USERNAME}")
...
def handle_docs(message):
    ...
    btn2 = telebot.types.InlineKeyboardButton("الشكل 2 (حقن فوك الأصلية - صافي)", callback_data="style_inject")
    markup.add(btn1, btn2)
    
    bot.reply_to(message, "اختار شكل الترجمة المفضل لدفعة التمريض:", reply_markup=markup)
...
# --- الشكل 2: الحقن المباشر (التعديل المصحح للصور اللي دزيتها) ---
...
def send_and_clean(message, out, inp):
    with open(out, 'rb') as f:
        bot.send_document(message.chat.id, f, caption=f"✅ تم الإنجاز بنجاح لدفعة أبطال التمريض🔥")
