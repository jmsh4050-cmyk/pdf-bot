...
def send_welcome(message):
    ...
    bot.reply_to(message, f"أهلاً {user_name}! تم تحديث البوت ليعمل بدون أخطاء نهائياً ✅\nأرسل ملفك الآن لدفعة التمريض.")
...
def handle_docs(message):
    ...
    markup = telebot.types.InlineKeyboardMarkup()
    btn1 = telebot.types.InlineKeyboardButton("1️⃣ شكل كلاسيك (مطور ضد الكراش 🛡️)", callback_data="style_fpdf")
...
# --- الحل النهائي للشكل 1: استخدام PyMuPDF لإنشاء الملف الجديد لتجنب خطأ latin-1 ---
def run_fpdf_style_fixed(message, file_info):
    ...
    try:
        ...
        for page in doc:
            ...
            # 1. معالجة الصور (منع التكرار)
            for img in page.get_images(full=True):
                xref = img[0]
                if xref in all_processed_images: continue
                try:
                    pix = fitz.Pixmap(doc, xref)
                    img_rect = fitz.Rect(70, y_offset, 350, y_offset + 180)
                    new_page.insert_image(img_rect, pixmap=pix)
                    y_offset += 200
                    all_processed_images.append(xref)
                except: pass

            # 2. معالجة النصوص (إنجليزي 14، عربي 14.5، عنوان 15)
                ...
                for line in lines:
                    ...
                    if len(line) > 3:
                        try:
                            ...
                            # كتابة الإنجليزي (تجاوز كامل لمشكلة latin-1)
                            new_page.insert_text((50, y_offset), line, fontsize=15 if is_header else 14, color=(0,0,0))
                            y_offset += 20
                            
                            # كتابة العربي
                            new_page.insert_text((50, y_offset), fixed_ar, fontsize=15 if is_header else 14.5, fontname="f0", fontfile=font_path, color=(0.8, 0, 0))
                            y_offset += 30
                        except: continue
...
# --- الأشكال الأخرى تبقى كما هي لأنها مستقرة ---
